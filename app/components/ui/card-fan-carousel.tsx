      ...
      <div className="flex items-center justify-center w-full">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full">
          {cards.map((card, index) => {
            ...
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card block">{image}</a>
            ) : (
      ...
      {needsPagination && (
        <div className="flex items-center justify-center gap-4 mt-8 z-30">
          ...
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <span key={i} className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${i === centerIndex ? "bg-black/60 scale-[1.3]" : "bg-black/15"}`} />
            ))}
          </div>
